...
function useCopyText(): CopyTextContent {
    ...
    if (!contextValue) {
        throw new Error(
            "CopyText.Message must be used within a CopyText component"
        );
    }
    return contextValue;
}
...
type ReactChildrenOrFunction<T> =
    | React.ReactNode
    | string
    | ((T: any) => React.ReactNode | string);

export function CopyTextMessage(props: {
    children: ReactChildrenOrFunction<boolean>;
}) {
...
export function CopyText(props: CopyTextProps) {
    ...
    return (
        <CopyTextContext.Provider value={{ handleCopy, showMessage }}>
            {React.createElement(
                as,
