...
We also used Sonnet 3.5 to assign a reward to the agent’s tool-use output. The reward rubric is tool-specific, and represents how effective the step was to solving the GitHub issue. For example, if the agent chooses to read a file that turns out to be irrelevant to solving the problem, that step is given a low reward. However, if the next file it chooses to read contains a critical clue, that step would be assigned a higher reward.
...
[SWE-search](https://arxiv.org/abs/2410.20285) clearly demonstrates the power of MCTS and using a reward function along with the feedback for exploring and exploiting the solution space. The key research here shows that by any basic framework can gain 20%+ improvement by guiding the framewok through MCTS. This is of paramout importance and shows that there is a lot to be gained by focussing on exploring the search space in a constructive manner and orchestrating the effort.
