...
We are calling this iteration of Aide as Aide(r+), it's a coding editor designed to talk and direct agents and keep the developer in the loop.
...
This is different from creating a custom environment or a shadow-workspace, we put the agent's work in-front of you, so you can direct it and help the agents when they get stuck and vice-versa ;)
...
This level of granularity allows us to not only make precise edits and it also keep the scope of “responsibility” for each agent very clear: “You are responsible for the code symbol, take any necessary steps to help the user query”
...
A challenge for agents at this level of granularity is that they are too removed from the context of the codebase. When presented with a single function in the codebase, even as developers we can't really think of all the ways this function is being used in the codebase .. it's near impossible without the codebase context. To alleviate that issue the agents tap into the editor itself specially the LSP (language server protocol)
